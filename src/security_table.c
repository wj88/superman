...
struct security_table* GetSecurityTable(void);
bool HasSecurityTableEntry(uint32_t ifindex, uint32_t daddr);

#define SECURITY_TABLE_MAX_LEN 1024

struct security_table {
	rwlock_t security_table_lock;
	struct list_head security_table_head;
	unsigned int security_table_total;
};

#define list_is_first(e) (&e->l == security_table_head.next)

struct security_table* GetSecurityTable()
{
	struct superman_net* snet = GetSupermanNet();
	return snet->security_table;
}
...
uint16_t GetNextTimestampFromSecurityTableEntry(uint32_t ifindex, uint32_t addr)
{
	...
	if(GetSecurityTableEntry(ifindex, addr, &entry))
	{
...
static inline void __security_table_flush(struct security_table* st)
{
	struct list_head *pos, *tmp;

	list_for_each_safe(pos, tmp, &st->security_table_head) {
		struct security_table_entry *e = (struct security_table_entry *)pos;
		list_del(&e->l);
		st->security_table_total--;
		kfree(e->sk);
		kfree(e->ske);
		kfree(e->skp);
		kfree(e);
...
static inline bool __security_table_add(struct security_table* st, struct security_table_entry *e)
{
	if (st->security_table_total >= SECURITY_TABLE_MAX_LEN) {
		printk(KERN_WARNING "SUPERMAN: security_table - \tMax list len reached (%d items).\n", SECURITY_TABLE_MAX_LEN);
		return false;
	}

	if (list_empty(&st->security_table_head)) {
		list_add(&e->l, &st->security_table_head);
	}
	else
	{
		list_add_tail(&e->l, &st->security_table_head);
	}
	return true;
}

static inline struct security_table_entry *__security_table_find(struct security_table* st, uint32_t ifindex, uint32_t daddr)
{
	struct list_head *pos;

	list_for_each(pos, &st->security_table_head) {
		struct security_table_entry *e = (struct security_table_entry *)pos;

		if (e->ifindex == ifindex && e->daddr == daddr)
			return e;
	}
	return NULL;
}

static inline bool __security_table_del(struct security_table* st, struct security_table_entry *e)
{
	if (e == NULL)
		return false;
...
{
	struct security_table* st = GetSecurityTable();
	if(!st)
		return false;

	struct security_table_entry *e;

	write_lock_bh(&st->security_table_lock);

	if ((e = __security_table_find(st, ifindex, daddr)) && __security_table_del(st, e))
	{
		if(e->sk)
			kfree(e->sk);
		if(e->ske)
		...
		e->skp = NULL;
		kfree(e);
		write_unlock_bh(&st->security_table_lock);
		return true;
	}
...
bool HasSecurityTableEntry(uint32_t ifindex, uint32_t daddr)
{
	...
	struct security_table_entry* entry;
	read_lock_bh(&st->security_table_lock);
	entry = __security_table_find(st, ifindex, daddr);
	read_unlock_bh(&st->security_table_lock);
	return entry != NULL;
}
...
bool GetSecurityTableEntry(uint32_t ifindex, uint32_t daddr, struct security_table_entry** entry)
{
	...
	*entry = __security_table_find(st, ifindex, daddr);
	...
	if(!AddSecurityTableEntry(ifindex, daddr, SUPERMAN_SECURITYTABLE_FLAG_SEC_NONE, 0, NULL, 0, NULL, 0, NULL, -1)) {
		return false;
	}
	else {
		read_lock_bh(&st->security_table_lock);
...
bool UpdateSecurityTableEntryFlag(uint32_t ifindex, uint32_t daddr, uint8_t flag, uint32_t timestamp)
{
	...
	if(GetSecurityTableEntry(ifindex, daddr, &e))
	{
		e->flag = flag;
		return true;
	}
	...
	{
		return UpdateOrAddSecurityTableEntry(ifindex, daddr, flag, 0, NULL, 0, NULL, 0, NULL, timestamp);
	}
}
...
bool AddSecurityTableEntry(uint32_t ifindex, uint32_t daddr, uint8_t flag, uint32_t sk_len, unsigned char* sk, uint32_t ske_len, unsigned char* ske, uint32_t skp_len, unsigned char* skp, int32_t timestamp)
{
	...
	if(!UpdateSecurityTableEntry(e, ifindex, daddr, flag, sk_len, sk, ske_len, ske, skp_len, skp, timestamp))
	{
		RemoveSecurityTableEntry(ifindex, daddr);
...
bool UpdateOrAddSecurityTableEntry(uint32_t ifindex, uint32_t daddr, uint8_t flag, uint32_t sk_len, unsigned char* sk, uint32_t ske_len, unsigned char* ske, uint32_t skp_len, unsigned char* skp, int32_t timestamp)
{
	...
	{
		return AddSecurityTableEntry(ifindex, daddr, flag, sk_len, sk, ske_len, ske, skp_len, skp, timestamp);
	}
}
...
bool UpdateBroadcastKey(uint32_t ifindex, uint32_t sk_len, unsigned char* sk, uint32_t ske_len, unsigned char* ske, uint32_t skp_len, unsigned char* skp, bool overwrite)
{
	...
	if(!overwrite && GetSecurityTableEntry(ifindex, INADDR_BROADCAST, &entry) && entry->flag >= SUPERMAN_SECURITYTABLE_FLAG_SEC_UNVERIFIED)
	{
	...
	return UpdateOrAddSecurityTableEntry(ifindex, INADDR_BROADCAST, flag, sk_len, sk, ske_len, ske, skp_len, skp, 0);
}

bool GetBroadcastKey(uint32_t ifindex, uint32_t* sk_len, unsigned char** sk)
{
	struct security_table_entry* entry;
	if(!GetSecurityTableEntry(ifindex, INADDR_BROADCAST, &entry))
		return false;
...
{
	if(!HasSecurityTableEntry(ifindex, INADDR_BROADCAST))
		return false;
	else
	{ 	struct security_table_entry* entry;
		if(!GetSecurityTableEntry(ifindex, INADDR_BROADCAST, &entry))
...
{
	snet->security_table = kmalloc(sizeof(struct security_table), GFP_KERNEL);
	if(!snet->security_table)
	{
		printk(KERN_WARNING "SUPERMAN security_table: unable to create the security table.\n");
		return false;
	}

	struct security_table* st = (struct security_table*)snet->security_table;

	INIT_LIST_HEAD(&st->security_table_head);
	st->security_table_lock = __RW_LOCK_UNLOCKED(st->security_table_lock);
	st->security_table_total = 0;

	// Add a broadcast key
	struct net* net = GetNet();
	struct net_device *dev;
	for_each_netdev(net, dev)
		UpdateBroadcastKey(dev->ifindex, 0, NULL, 0, NULL, 0, NULL, true);
	put_net(net);
...
void DeInitSecurityTable(struct superman_net* snet)
{
	...
	{
		write_lock_bh(&st->security_table_lock);
		__security_table_flush(st);
		write_unlock_bh(&st->security_table_lock);

		kfree(st);
		snet->security_table = NULL;
	}
}
